...
const DEFAULT_TERMS: { term: string; def: string }[] = [
...
export default function GlossaryContent() {
  const cms = useContent("glossary");
  const terms = cms.list<{ term: string; def: string }>("terms", "items", DEFAULT_TERMS);
      ...
      <section style={{ padding: "0 0 120px" }}>
        <div className="container" style={{ maxWidth: 880 }}>
          <div className="glossary-list">
            {terms.map((t, i) => (
              <div className="glossary-item" key={t.term || i}>
                <h3>{t.term}</h3>
                <p>{t.def}</p>
              </div>
            ))}
